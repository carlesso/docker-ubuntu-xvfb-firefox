...
LABEL Description="Ruby 2.2.1 image with xorg and xvfb. Useful to run test with selenium"
...
RUN mkdir /tmp/ruby
RUN cd /tmp/ruby && curl --silent ftp://ftp.ruby-lang.org/pub/ruby/2.2/ruby-2.2.1.tar.gz | tar xz
RUN cd /tmp/ruby/ruby-2.2.1 && ./configure --disable-install-rdoc && make install
